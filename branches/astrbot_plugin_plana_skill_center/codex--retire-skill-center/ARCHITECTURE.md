# Plana Skill Center 归档架构

> 本仓库已于 2026-07-21 退役。`main.py` 仅保留无副作用的 AstrBot 兼容类，不导入 `plugin/runtime.py`，不初始化 SQLite，也不注册命令、Web API 或 LLM tool。以下内容描述退役前实现，供数据审计和迁移参考。
...
- `main.py`: 已退役的 inert AstrBot 兼容入口，不加载历史 runtime。
