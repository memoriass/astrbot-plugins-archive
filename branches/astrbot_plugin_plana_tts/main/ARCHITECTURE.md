...
2. `voice/runtime.py` normalizes plugin config. When `enable_core_api=true`, it
   registers `/plana_tts/state` and `/plana_tts/synthesize`.
3. Plana Core calls `/plana_tts/synthesize` with contract
   `plana.voice.synthesis.v1`, text, session origin, and message type.
4. The plugin validates its own switch, loopback-only Core API boundary,
...
- `voice/runtime.py`: runtime configuration, command handlers, Core HTTP API,
  auth checks, validation, and engine dispatch.
