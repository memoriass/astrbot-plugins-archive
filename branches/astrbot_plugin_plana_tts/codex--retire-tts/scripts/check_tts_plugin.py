...
def main() -> None:
    for rel in (
        ...
        "voice/runtime.py",
        "voice/core_service.py",
        "voice/core_server.py",
    ...
    for snippet in ("RETIRED:", "retired", "Plana TTS (Retired)"):
        require(snippet in metadata, f"metadata_retirement_missing={snippet}")
    main_py = (ROOT / "main.py").read_text(encoding="utf-8")
    require("class PlanaTTSPlugin(Star)" in main_py, "retired_shell_missing")
    require("from .voice" not in main_py, "legacy_runtime_still_loaded")
    for forbidden in (
        "filter.",
        "register_web_api",
        "add_llm_tools",
        ...
        "AudioStorage",
        "PlanaTTSLoopbackServer",
    ):
        require(forbidden not in main_py, f"retired_entry_side_effect={forbidden}")
    ...
    core_server = (ROOT / "voice" / "core_server.py").read_text(encoding="utf-8")
    require('"127.0.0.1"' in core_server, "core_server_not_loopback")
    require("X-Plana-Core-Key" in core_server, "core_service_key_missing")
    require(
        "self.core_service = core_service if self.enable_core_api else None"
        in runtime_py,
        "core_service_switch_not_enforced",
    )
