# Plana TTS Archived Architecture

> This repository was retired on 2026-07-21. `main.py` now provides an inert AstrBot compatibility class and does not import `voice/runtime.py`, create audio storage, register handlers or services, or start the loopback server. The remaining document describes the historical implementation for audit purposes.
...
1. `main.py` exposes only the retired compatibility shell; the historical runtime is not imported.
...
- `main.py`: retired inert AstrBot entry; it must not import the historical runtime.
