...
启用 `enable_core_api` 后，本插件暴露受控 `core_service`。Core 与 TTS 位于同一 AstrBot 进程时优先直接调用该 Service，不经过 HTTP，也不需要服务密钥。

原有 AstrBot Dashboard 路由仍可注册用于管理和兼容：
...
只有 Core 与 TTS 分进程运行时才启用专用 loopback HTTP。TTS 设置 `core_service_http_enabled=true` 和 `core_service_key`，Core 设置相同的 `plana_core_service_key`，并将 `ops_voice.voice_synthesis_url` 指向 `http://127.0.0.1:6191/plana_tts/synthesize`。请求头固定为 `X-Plana-Core-Key`；服务只绑定 `127.0.0.1`、默认关闭，不复用 Dashboard JWT 或外部 TTS Token。

## 配置

- `enabled`: 总开关，默认 `false`。
- `enable_core_api`: 是否开放给 Plana Core 的受控合成入口，默认 `false`。
- `core_service_http_enabled`: 是否开启 Core 专用 loopback HTTP，默认 `false`；同进程调用不需要开启。
- `core_service_port`: 专用回环端口，默认 `6191`。
- `core_service_key`: 分进程时与 Core `plana_core_service_key` 一致的共享密钥。
...
core_service_http_enabled = true
core_service_key = <same value as Core plana_core_service_key>
```
