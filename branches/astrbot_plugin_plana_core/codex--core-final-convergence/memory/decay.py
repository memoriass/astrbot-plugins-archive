...
class MemoryDecay:
    ...
    def decay_scope(self, scope_id: str) -> DecayReport:
        ...
        for memory in candidates:
            ...
            report.processed += 1
            report.decayed += 1
        if hasattr(self.storage, "expire_stale_atoms"):
            report.atom_expired = self.storage.expire_stale_atoms(scope_id)
        if hasattr(self.storage, "forget_expired_atoms"):
            report.atom_forgotten = self.storage.forget_expired_atoms(7.0, scope_id)
        return report
