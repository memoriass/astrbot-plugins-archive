"""Memory scope management — alias resolution and cross-scope migration.
        ...
        *,
        ...
        with self.db.connect() as conn:
            ...
            for row in source_rows:
                src_id, kind, content, importance, source, created_at = row
                if content in target_contents:
                    ...
                    continue
                conn.execute(
                    """INSERT INTO episodic_memories(scope_id, kind, content, importance, source, created_at)
                       VALUES (?, ?, ?, ?, ?, ?)""",
                    (
                        target_scope,
                        kind,
