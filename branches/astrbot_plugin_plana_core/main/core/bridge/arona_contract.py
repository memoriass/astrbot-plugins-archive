...
from ..models import AronaTaskRequest
...
class AronaContract:
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "bridge_plugin": "builtin (core/bridge/nacho_sidecar)",
    ...
    def disabled_result(self) -> dict[str, Any]:
        return {
            "ok": False,
            "error": "arona_api_disabled",
            "status": self.status(),
        }

    def normalize_task(self, payload: dict[str, Any]) -> AronaTaskRequest:
        task_payload = (
            payload.get("payload") if isinstance(payload.get("payload"), dict) else {}
        )
        return AronaTaskRequest(
            ...
            source=str(payload.get("source", "arona")),
    ...
    def describe_task(self, task: AronaTaskRequest) -> dict[str, Any]:
        return asdict(task)
    ...
    def result_report(
        ...
        return {
            ...
            "source": "plana",
            "target": request.get("source", "arona"),
            "result": result,
            "created_at": int(time()),
        }

    def context_sync(self, scope_id: str, items: list[str]) -> dict[str, Any]:
        return {
            "ok": True,
            "kind": "context_sync",
            "source": "plana",
            "scope_id": scope_id,
            "memory_context": items[:8],
            "created_at": int(time()),
        }
