...
def _check_metadata() -> None:
    ...
    _check_register_version(metadata.get("version", ""))
    _check_plugin_logo()
...
def _check_register_version(metadata_version: str) -> None:
    ...
    if not match:
        raise SystemExit("main.py @register version is missing")
    if metadata_version != match.group(1):
        raise SystemExit(
            f"metadata.yaml version {metadata_version} does not match @register {match.group(1)}"
        )
