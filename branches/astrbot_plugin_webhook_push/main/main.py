...
class Main(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        import shutil
        # 初始化基础资源目录：如果用户目录不存在，则从插件自带的 utils/defaults/ 复制过来
        default_resource_path = Path(__file__).parent / "utils" / "defaults"
        for bg_dir in ["media_bg", "game_bg", "common_bg"]:
            target_dir = base_data_path / bg_dir
            source_dir = default_resource_path / bg_dir
            if not target_dir.exists():
                if source_dir.exists():
                    try:
                        shutil.copytree(source_dir, target_dir)
                        logger.info(f"已初始化覆盖默认背景文件夹: {bg_dir}")
                    except Exception as e:
                        logger.error(f"复制默认背景图 {bg_dir} 失败: {e}")
