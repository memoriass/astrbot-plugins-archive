...
from .alias_manager import GameAliasManager
...
class GameHandler:
    ...
    def __init__(self, context, config: dict = None):
        ...
        # 初始化别名管理器
        self.alias_manager = GameAliasManager(base_data)
    ...
    def _get_random_bg_for_source(self, source: str) -> str:
        ...
        # 搜寻逻辑：
        # 对 AI 识别出来的模糊字符串进行规范化
        search_prefix = self.alias_manager.get_canonical_name(source)
    ...
    def detect_game_source(self, payload: dict, headers: dict = None) -> str:
        ...
        if headers and "user-agent" in headers:
            ...
            if "python-requests" in ua:
                # 如果是 python 请求且带有 title/message/desp 字段，极大概率是这类脚本
                if "title" in payload and ("message" in payload or "content" in payload or "desp" in payload):
                    # 再次尝试从 title 判断
